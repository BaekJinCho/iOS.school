...
: 특정 개발 방법론을 가르키는 말은 아니고 애자일(기민한, 좋은것을 빠르고 낭비없게 만드는 것) **개발을 가능하게 해 주는 다양한 방법론 전체를 일컫는 말**
...
: 단순히 다이어그램을 그리는 것이 아니고 이것을 표준을 맞게 하면 코드를 만들어준다.
...
: 매우 짧은 개발 사이클을 개발하는 소프트웨어 개발 프로세스

: 코드 품질을 관리하며 개발을 하는 방법

	1) 하나의 코드를 짜고
	2) 결함을 점검하는 자동화된 테스트 케이스를 작성
...
4. **PDD(Plan-Driven Development)** -> 고전적인 방법
...
: 다중 스레드
 ...
 ex) 화면에 애니메이션도 보여주고 성우가 책을 읽어주는 것도 표현해야 한다.

	- 멀티 스레드 vs 멀티 태스크

: cpu에 한번에 여러가지 프로세스를 처리하는 환경

 ex) 노래를 들으며 책을 읽는다. 밥을 먹으며 이야기를 한다.
...
- iOS
- 
: Cocoa Touch 프레임워크를 통해 App을 만들어 볼 수 있다.
...
: 과거 소프트웨어 개발 과정에서 발견된 노하우에 이름을 붙여 이후에 재 이용하기 좋은 형태로 묶어 정리한 것
	...
	- 1) MVC(Model-View-Controller)
		- Model : 자료 저장소(데이터 관리)
		- Controller : 중재자
		- View : 화면 표시 될 역할을 담당할 클래스<br>ex) 버튼, 텍스트, 웹 뷰
...
- 래퍼런스 문서(Reference Document) - 항상 옆에두고 봐야한다!!!!
